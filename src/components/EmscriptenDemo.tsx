...
type ModuleOptions = {
  ...
  requestFullscreen?(pointerLock: boolean, resizeCanvas: boolean): void;
};
...
export default function EmscriptenDemo() {
  ...
  const [moduleOptions, setModuleOptions] = useState<ModuleOptions>(
    {} as ModuleOptions
  );
  const [progressObject, setProgressObject] = useState<{
    ...
    max: 100,
    hidden: true,
  } as { value: number; max: number; hidden: boolean });
  const [spinnerIsHidden, setSpinnerIsHidden] = useState(false);
  ...
  useEffect(() => {
    ...
    const moduleOptions: ModuleOptions = {
      ...
      setStatus(text: string): void {
        moduleOptions.setStatus.last ??= { time: Date.now(), text: "" };
        if (text === moduleOptions.setStatus.last.text) return;
        const m = RegExp(/([^(]+)\((\d+(\.\d+)?)\/(\d+)\)/).exec(text);
        const now = Date.now();
        // if this is a progress update, skip it if too soon
        if (m && now - moduleOptions.setStatus.last.time < 30) return;
        moduleOptions.setStatus.last.time = now;
        moduleOptions.setStatus.last.text = text;
        if (m) {
          text = m[1];
          setProgressObject({
            value: parseInt(m[2]) * 100,
            max: parseInt(m[4]) * 100,
            hidden: false,
          });
          ...
          if (!text) setSpinnerIsHidden(true);
        }
        setLocalStatus(text);
      },
      totalDependencies: 0,
    ...
    moduleOptions.setStatus("Downloading...");
    window.onerror = () => {
      // TODO: do not warn on ok events like simulating an infinite loop or exitStatus
      moduleOptions.setStatus("Exception thrown, see JavaScript console");
      setSpinnerIsHidden(true);
      moduleOptions.setStatus = (text) => {
        if (text) console.error("[post-exception status] " + text);
      };
    };

    Module(moduleOptions).then((mod: MainModule) => {
      console.log(mod);
  ...
  const requestFullscreen = () => {
    if (moduleOptions.requestFullscreen) {
      moduleOptions.requestFullscreen(isPointerLock, isResize);
    }
  };
    ...
    <>
      ...
      <span id="controls">
        ...
        <label>
          <input
            type="checkbox"
            checked={isPointerLock}
            onChange={(e) => setIsPointerLock(e.target.checked)}
          />
      ...
      <div className="emscripten">
        <progress
          value={progressObject.value}
          max={progressObject.max}
          id="progress"
          hidden={progressObject.hidden}
        ></progress>
      </div>
      ...
      <textarea id="output" rows={8} value={output}></textarea>
    </>
  );
}
